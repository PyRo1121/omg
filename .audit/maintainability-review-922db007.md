...
- **Arch decreasing identity:** `src/package_managers/pacman_db/db.rs:1025`; `alpm_direct.rs:36`; `alpm_worker.rs:53`. Cached handles refresh on increasing timestamps, not changed identities. Removing the last database or restoring older data can retain stale handles. Test identity inequality while preserving explicit invalidation.
