...
impl Transaction {
    ...
    async fn download_packages(&mut self) -> Result<()> {
        let client = reqwest::Client::builder()
            .timeout(Duration::from_secs(300))
            .connect_timeout(Duration::from_secs(10))
            // OPTIMIZATION: Aggressive connection pooling for parallel downloads
            .pool_max_idle_per_host(MAX_CONCURRENT_PACKAGE_DOWNLOADS * 2)
            .pool_idle_timeout(Duration::from_secs(120)) // Keep connections alive longer
            .tcp_keepalive(Duration::from_secs(60)) // TCP keepalive for long downloads
            .tcp_nodelay(true) // Disable Nagle's algorithm for lower latency
            // NOTE: Don't force HTTP/2 - Ubuntu mirrors don't support it properly
            .build()?;
        ...
        let total_packages = packages_to_download.len();
        tracing::info!(
            "Downloading {} packages in parallel (max {} concurrent)",
            total_packages,
            MAX_CONCURRENT_PACKAGE_DOWNLOADS
        );
        ...
        // Clone content store for use in async tasks
        let content_store = self.content_store.clone();
        ...
        let download_tasks =
            packages_to_download
                ...
                .map(|(idx, name, version, url, sha256)| {
                    ...
                    pb.set_style(
                ProgressStyle::default_bar()
                    .template("  {prefix:.cyan} [{bar:25.green/blue}] {bytes}/{total_bytes} {msg}")
                    .expect("valid template")
                    .progress_chars("=>-"),
            );
                    pb.set_prefix(name.clone());
                    ...
                    async move {
                        let result = download_package_with_retry(
                            &client,
                            &name,
                            &version,
                            &url,
        ...
        // Check for failures and update package actions
        let mut downloaded_paths: HashMap<usize, PathBuf> = HashMap::new();
        ...
        let packages: Vec<_> = self
            ...
            .filter_map(|action| {
                action
                    .deb_path
                    .as_ref()
                    .map(|p| (p.clone(), action.name.clone()))
            })
            .collect();

        for (deb_path, name) in packages {
            self.unpack_deb(&deb_path, &name)?;
        }
        Ok(())
    }
    ...
    fn unpack_deb(&mut self, deb_path: &Path, package_name: &str) -> Result<()> {
        ...
        while let Some(entry) = archive.next_entry() {
            ...
            if name.starts_with("control.tar") {
                tracing::debug!("Found control.tar in {}", package_name);
                control_tar = Some(contents);
            } else if name.starts_with("data.tar") {
                tracing::debug!(
                    "Found data.tar in {} (size: {} bytes)",
                    package_name,
                    contents.len()
                );
        ...
        if let Some(data) = data_tar {
            let files = extract_tar_to_root(&data)?;
...
/// Download a single package with retry logic and progress reporting
///
/// Checks the content store first for deduplication. If the package exists in
/// the store (by SHA256 hash), a hard link is created instead of downloading.
        ...
        match download_package_once(client, url, &dest, progress).await {
            Ok(()) => {
                ...
                match content_store.store(&dest) {
                    Ok(hash) => {
                        tracing::debug!(
                            "Stored {} in content store with hash: {}",
                            name,
                            &hash[..8]
                        );
                    }
                    Err(e) => {
                        ...
                        tracing::warn!("Failed to store {} in content store: {}", name, e);
                    }
                }
...
/// Download a package once with progress tracking
...
async fn download_package_once(
    ...
    // Write to disk
    fs::write(dest, &bytes)
        .with_context(|| format!("Failed to write .deb file to {}", dest.display()))?;
