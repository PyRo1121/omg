...
impl AurIndex {
    ...
    pub fn open(path: &Path) -> Result<Self> {
        ...
        // SAFETY: Memory mapping requires unsafe, but we ensure:
        // 1. File is opened read-only
        // 2. File handle lives as long as mmap
        // 3. File is not modified externally during use
    ...
    fn archive(&self) -> Result<&ArchivedAurArchive> {
        rkyv::access::<ArchivedAurArchive, rkyv::rancor::Error>(&self.mmap)
