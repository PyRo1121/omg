...
pub async fn init(team_id: &str, name: Option<&str>) -> Result<()> {
    ...
    println!();
    println!("Next steps:");
    println!("  1. Run {} to capture your environment", "omg env capture".cyan());
...
pub async fn members() -> Result<()> {
    ...
    println!("  Team: {} ({})", status.config.name.cyan(), status.config.team_id.dimmed());
    println!();
...
fn print_status(status: &TeamStatus) {
    ...
    println!("  Lock hash: {}", if status.lock_hash.is_empty() {
    ...
    for member in &status.members {
        ...
        println!(
            ...
            sync_icon,
            member.name,
            if member.in_sync { "in sync".green().to_string() } else { "drift detected".yellow().to_string() }
        );
    }
