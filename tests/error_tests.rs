...
mod invalid_input_errors {
    ...
    fn test_invalid_package_name_error() {
        ...
        assert!(!success || stdout.contains("not found"),
...
mod database_errors {
    ...
    fn test_missing_database_creates_new() {
        ...
        let (success, stdout, stderr) = run_omg_with_env(
            &["status"],
            &[("OMG_DATA_DIR", data_dir.to_str().unwrap())]
        );
...
mod config_errors {
    ...
    fn test_invalid_lock_file_error() {
        ...
        let (success, stdout, stderr) = run_omg_in_dir(
            &["env", "check"],
            temp_dir.path()
        );
...
mod panic_prevention {
    ...
    fn test_empty_query_does_not_panic() {
        ...
        assert!(!success || _stdout.len() > 0 || _stderr.len() > 0,
    ...
    fn test_special_chars_do_not_panic() {
        let (success, _stdout, _stderr) = run_omg(&["search", "\x00\x01\x02"]);
        assert!(!success || _stdout.len() > 0 || _stderr.len() > 0,
    ...
    fn test_unicode_search_does_not_panic() {
        let (success, _stdout, _stderr) = run_omg(&["search", "日本語한글中文"]);
        assert!(!success || _stdout.len() > 0 || _stderr.len() > 0,
