...
async fn test_env_capture_and_check_success() -> Result<()> {
    ...
    let ctx = get_ctx();
    let capture_cmd = Commands::Env { command: EnvCommands::Capture };
    
    capture_cmd.execute(&ctx).await?;
    assert!(temp.path().join("omg.lock").exists());

    let check_cmd = Commands::Env { command: EnvCommands::Check };
...
async fn test_env_check_fails_without_lock() -> Result<()> {
    ...
    assert!(result.unwrap_err().to_string().contains("No omg.lock file found"));
...
async fn test_tool_list_empty() -> Result<()> {
    ...
    let ctx = get_ctx();
    let list_cmd = Commands::Tool { command: ToolCommands::List };
...
async fn test_tool_registry_output() -> Result<()> {
    let ctx = get_ctx();
    let reg_cmd = Commands::Tool { command: ToolCommands::Registry };
...
async fn test_tool_install_invalid_name_fails() -> Result<()> {
    let ctx = get_ctx();
    let install_cmd = Commands::Tool { command: ToolCommands::Install { name: "../dangerous".to_string() } };
...
async fn test_fleet_status_requires_license() -> Result<()> {
    let ctx = get_ctx();
    let status_cmd = Commands::Fleet { command: FleetCommands::Status };
...
async fn test_fleet_push_invalid_team_fails() -> Result<()> {
    ...
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("Invalid team identifier"));
    Ok(())
}
...
async fn test_run_invalid_task_fails() -> Result<()> {
    ...
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("Invalid task name"));
    Ok(())
}
...
async fn test_tool_install_not_in_registry_fails() -> Result<()> {
    let ctx = get_ctx();
    let install_cmd = Commands::Tool { command: ToolCommands::Install { name: "non-existent-tool-xyz-123".to_string() } };
