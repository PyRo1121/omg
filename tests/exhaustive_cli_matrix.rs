...
fn run_debian(args: &[&str]) -> CommandResult {
    run_omg_with_env(args, &[("OMG_TEST_DISTRO", "debian"), ("OMG_TEST_MODE", "1")])
}
...
mod arch_matrix {
    ...
    fn test_install_remove_cycle() {
        ...
        let envs = &[("OMG_TEST_DISTRO", "arch"), ("OMG_TEST_MODE", "1"), ("OMG_DATA_DIR", data_path)];
...
mod debian_matrix {
    ...
    fn test_install_remove_cycle() {
        let data_dir = TempDir::new().unwrap();
        let data_path = data_dir.path().to_str().unwrap();
        let envs = &[("OMG_TEST_DISTRO", "debian"), ("OMG_TEST_MODE", "1"), ("OMG_DATA_DIR", data_path)];
...
mod runtime_matrix {
    ...
    fn test_env_workflow() {
        ...
        let entries = std::fs::read_dir(project.path()).unwrap()
        ...
        // Check
        project.run_with_env(&["env", "check"], envs).assert_success();
    }
