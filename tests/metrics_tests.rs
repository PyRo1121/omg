...
async fn test_metrics_collection() {
    ...
    assert_eq!(after_ping.requests_total, initial.requests_total + 1, "Total requests should inc by 1");
    assert_eq!(after_ping.requests_failed, initial.requests_failed, "Failed requests should stay same");
    ...
    assert_eq!(after_invalid.requests_total, initial.requests_total + 2, "Total requests should inc by 2");
    assert_eq!(after_invalid.requests_failed, initial.requests_failed + 1, "Failed requests should inc by 1");
    assert_eq!(after_invalid.validation_failures, initial.validation_failures + 1, "Validation failures should inc by 1");
    ...
    if let Response::Success { result: ResponseResult::Metrics(snapshot), .. } = response {
...
async fn test_security_audit_metrics() {
    ...
    assert_eq!(after.security_audit_requests, initial.security_audit_requests + 1);
}
